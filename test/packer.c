...
define_test(single_channel_no_resample, {
    ...
    pass(mixed_make_segment_packer(&pack_i, pack_o.samplerate, &packer));
    pass(mixed_make_segment_unpacker(&pack_o, pack_i.samplerate, &unpacker));
    // Connect the buffers
...
define_test(single_channel_resample, {
    struct mixed_packed_audio pack_i = {0};
    struct mixed_packed_audio pack_o = {0};
    struct mixed_buffer buffer = {0};
    struct mixed_segment packer = {0};
    ...
    pass(mixed_make_segment_packer(&pack_i, 75, &packer));
    pass(mixed_make_segment_unpacker(&pack_o, 75, &unpacker));
    // Connect the buffers
    pass(mixed_segment_set_out(MIXED_BUFFER, MIXED_MONO, &buffer, &unpacker));
    pass(mixed_segment_set_in(MIXED_BUFFER, MIXED_MONO, &buffer, &packer));
    // Fill data
    ...
    pass(mixed_segment_mix(&packer));
    // Check
    is(pack_i.frames, pack_o.frames);
    for(int i=0; i<pack_i.size; ++i)
      is(data_i[i], data_o[i]);
