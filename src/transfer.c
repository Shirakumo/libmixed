...
#define DEF_MIXED_TRANSFER_ARRAY_FROM_ALTERNATING(datatype)             \
  VECTORIZE void mixed_transfer_array_from_alternating_##datatype(void *in, float *out, uint8_t stride, uint32_t samples, float volume) { \
...
#define DEF_MIXED_TRANSFER_ARRAY_TO_ALTERNATING(datatype)               \
  VECTORIZE static inline void mixed_transfer_array_to_alternating_##datatype(float *in, void *out, uint8_t stride, uint32_t samples, float volume){ \
    for(uint32_t sample=0; sample<samples; ++sample){                     \
